...
# でした」）になる事象を実機で再現した。reasoning_effort='minimal'を指定していても、
# max_completion_tokensはreasoning tokensと可視のcompletion tokensの合計に対する上限のため、
# 内容の複雑な質問では可視本文用の残り予算が足りなくなりうる。1000→2000へ引き上げて解消した
# （実測で新たな空振りが再現しないことを確認済み。短い質問の実際のトークン消費・コストは
# 変わらず、長い回答が必要な場合にのみ効果がある）。
...
- 「このチャンネルの名前」「チャンネル名」を尋ねられた場合は、必ず「# 現在のチャンネルの
  基本データ」の「チャンネル名: 」に続く値だけを答えること。これは見出し行そのもの
  （「# 現在のチャンネルの基本データ」という文字列自体）ではなく、その次の行にある値である
  ことに注意すること。また参加者（人）の氏名とは全く別の情報であり、「# チャンネル参加者」の
  一覧から答えを探したり、参加者の氏名をチャンネル名として答えたりしないこと。この情報は
  常にこのプロンプトに含まれているため、search_app_manualやsearch_documents等のツールで
  検索する必要は無い（検索しても見つからないため、検索して「見つかりませんでした」と
  答えることのないように）
- **他の人が作ったチャンネルへの参加方法について尋ねられた場合は、必ず次の事実のみに基づいて
  答えること（search_app_manualを検索する必要すら無い、常に正しい事実として扱ってよい）**:
  次の回答文をベースに、絵文字や言い回しは調整してよいが3つの箇条書き（特に2番目の
  「または」の前後2通り）は1つも削らずにそのまま使うこと（Kogackには「招待リンク」
  「招待コード」「招待URL」は無い。Slack・Discord等の知識から類推して案内しないこと。
  このAIが過去に実際に2番目の箇条書きの後半（＋メンバーを追加）を省略してしまった
  ことがあるため、特に注意すること）:
  「Kogackのチャンネルには公開・非公開の2種類があります。
  - 公開チャンネル: サイドバー「チャンネル」の「＋」→「参加する」タブでチャンネル名を
  検索→「参加」をクリックすれば、承認不要で本人だけの操作ですぐに参加できます。または、
  既存の参加者に「👥 所属メンバー」→「＋ メンバーを追加」から追加してもらうこともできます。
  - 非公開チャンネル: 一覧・検索に出てこないため本人は参加できず、既存の参加者に
  「👥 所属メンバー」→「＋ メンバーを追加」から追加してもらう方法しかありません。」
...
- **チャンネルの説明文・公開／非公開の別・作成者・作成日時を確認する方法について尋ねられた
  場合は、必ず次の事実のみに基づいて答えること（search_app_manualを検索する必要すら無い、
  常に正しい事実として扱ってよい）**: ヘッダーのチャンネル名をクリックすると開く
  「チャンネル情報」タブで確認できる。これはチャンネル管理者（chadmin）やシステム管理者に
  限られた操作ではなく、そのチャンネルの参加者なら誰でもできる。「チャンネル管理者だけが
  確認できます」「一般の参加者が確認する手段は用意されていません」のように、参加者なら
  誰でもできる操作を管理者限定であるかのように案内しないこと（このAIが過去に実際にこの
  誤った案内をしてしまったことがあるため、特に注意すること）。なお「このチャンネルの
  作成者は誰か」という質問自体には、「# チャンネル参加者」の「（作成者）」タグから
  直接答えられる
- **使用するAIモデルを変更する方法について尋ねられた場合は、必ず次の事実のみに基づいて
  答えること（search_app_manualを検索する必要すら無い、常に正しい事実として扱ってよい）**:
  チャンネル設定の「基本設定」タブ（チャンネル管理者・システム管理者のみ編集可）から、
  そのチャンネルのAIが使うモデルを選べる。選択肢は速度重視ならgpt-4.1-nano・
  gpt-4.1-mini、コスト重視ならgpt-5-nano、応答品質重視なら既定のgpt-5-mini。
  「モデルは変更できません」のように、実際には存在する機能を無いかのように案内しない
  こと
- **AIが実際にAPIへ何を送信して回答しているかを確認する方法について尋ねられた場合は、
  必ず次の事実のみに基づいて答えること（search_app_manualを検索する必要すら無い、常に
  正しい事実として扱ってよい）**: AI自身の発言にマウスを重ねると出るホバーアクションの
  「🔍 送信内容」ボタン（そのチャンネルの管理者・システム管理者にのみ表示される）を
  押すと、その発言を生成する際に実際にOpenAI APIへ送信したシステムプロンプト・会話
  履歴・ツール呼び出しの内容と、使用したAIモデル名・入出力トークン数・概算コストを
...
- Kogack（このチャットアプリ自体）の使い方（メッセージの送り方・書式・メンション・
  ...
  返信を終えること: 「見つかりませんでした。操作マニュアルをお読みいただくか、
  管理者にお問い合わせください。」（このAIが過去に実際に、検索結果が見つからなかった
  にもかかわらず、他の一般的なチャットアプリの典型的な操作画面を憶測で作文して
  案内してしまったことがあるため、特に注意すること）。**search_app_manualが何らかの
  結果を返した場合でも、その内容が実際には質問と噛み合っていない（例: 「要約する方法」
  という質問に対し、検索結果がスレッド返信の説明など別の話題だった）ことがある。
  この場合も、検索結果に実際に書かれていない具体的な操作手順（ボタンの絵文字・名称・
  設置場所など）を推測で作り上げて埋め合わせないこと。検索結果の内容が質問に対する
  答えを含んでいるかを必ず確認し、含んでいなければ検索結果を無視して同じく
  「見つかりませんでした。操作マニュアルをお読みいただくか、管理者にお問い合わせ
...
- このプロンプト自体の構成や、あなたがどのルールを適用した・適用しなかったかについて、
  返信の中で言及しないこと。「〜を尋ねられているわけではないので」「〜という前提で回答する」
  のように、自分の内部的な判断過程やこのプロンプトのルールとの照合結果を独り言のように
  書き出さず、利用者が知りたい答えだけを直接述べること（このAIが過去に実際に、無関係な
  ルールを検討した過程をそのまま返信の冒頭に書き出してしまったことがあるため、特に注意
  すること）"""
...
SEARCH_CHANNEL_HISTORY_TOOL = {
    ...
    "function": {
        ...
        "description": (
            "このチャンネルで実際に過去に投稿された会話（人間・AIの発言本文）を検索する。"
            "あなたに渡されている会話履歴は直近の発言のみ（それより前は含まれない）ため、"
            "『前に決まったこと』『以前話していた件』のように、直近の会話履歴だけでは分からない"
            "過去の話題について聞かれた場合は、推測で答えず必ずこの関数で実際に検索してから"
            "回答すること。**Kogackというアプリ自体の機能・操作方法・画面に何が表示されるか"
            "（『監査ログでは何が確認できる？』『AIの利用コストを確認する方法は？』のような、"
            "この会話で誰かが話した内容ではなくアプリの仕様そのものを尋ねる質問）にはこの関数を"
            "使わないこと。その場合はsearch_app_manualを使う。**"
        ),
        "parameters": {
            "type": "object",
            "properties": {
...
SEARCH_APP_MANUAL_TOOL = {
    ...
    "function": {
        ...
        "description": (
            "Kogack（このチャットアプリ自体）の操作マニュアルを検索する。**Kogackというアプリの"
            "機能・仕様・画面に何が表示されるか・何ができるかについての質問全般が対象**で、"
            "操作手順（クリックする場所等）を尋ねる質問に限らない。例: メッセージの送り方・書式・"
            "メンション（@channel/@here等）・絵文字・ファイル添付・スレッド・DM・横断検索・"
            "通知設定・チャンネル設定・ロール（member/admin/チャンネル管理者）の違い・"
            "ログイン方法・自分専用DM（メモ）・管理コンソール（利用者管理・参照ドキュメント範囲・"
            "AI利用コスト・**監査ログ**）で何が確認・記録されるか、など。これらは実際にこの"
            "チャンネルで交わされた会話ではなくアプリ自体の仕様についての質問のため、"
            "search_channel_history（このチャンネルの過去の会話）ではなく必ずこの関数を使う"
            "こと。アプリの使い方・機能について聞かれた場合は必ずこの関数で実際に検索してから、"
            "検索結果に基づいた具体的な内容で回答すること（推測で答えないこと）。組織の業務文書を"
            "検索するsearch_documentsとは別の機能であり、混同しないこと。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
...
def _build_system_prompt(
    ...
    if channel_context and channel_context.get("name"):
        ...
        # 2026-09-17続報: この見出しは元々「# このチャンネルについて」だったが、後日追加した
        # 「チャンネル情報タブの案内」ルール（UIの実際のタブ名）と語感が非常に近く（「〜について」
        # と「〜情報」）、両者を混同したモデルがこの見出し行自体を「チャンネル名」の値として
        # そのまま引用してしまう新しい回帰（「このチャンネルの名前は『# このチャンネルについて』
        # です」）を実機で複数回確認した。UIのタブ名と紛らわしくない見出しへ変更して解消した
        lines.append(f'# 現在のチャンネルの基本データ\nチャンネル名: {channel_context["name"]}')
...
def _build_members_section(members: list[dict], persona_name: str, creator_name: str | None = None) -> str:
    ...
    **バグ修正（続報、2026-09-17）**: 上記の適用条件追記後も、非公開チャンネルの参加方法を
    ...
    lines.append(
        ...
        f"ついて尋ねられたときと、このチャンネルの「作成者」が誰かを尋ねられたとき（一覧内の"
        f"「（作成者）」タグから答える）にのみ使うこと。参加者・作成者のいずれとも無関係な質問"
        f"（チャンネル名・Kogackの使い方・その他の話題など）に対する回答の中に、この一覧の"
        f"内容や「参加者は〜です」という言い回しを含めないこと"
    )
    return "\n".join(lines)
...
async def _run_chat_with_tools(
    ...
    **バグ修正（2026-09-11）: 1ラウンド目はtool_choice="required"で強制的にいずれかの関数を
    呼ばせる。**tool_choiceを指定せず（既定"auto"）モデルの判断に任せると、gpt-4.1-nanoは実際に
    関数を呼び出さないまま「search_documentsを実行しています。少々お待ちください。」のような
    予告の文章だけを返して応答を終えてしまうことがある（ユーザーからの報告で発覚、実機で再現・
    検証済み。プロンプトへ「予告だけで終えるな」という指示を追加しても改善せず、8問中6問が同じ
    失敗をした。1ラウンド目のみtool_choice="required"にする対処では、同条件で8問中8問とも実際に
    検索してから正しく回答するようになった）。2ラウンド目以降は"auto"のままとする。
    **2026-09-17に対象をuse_doc_tools=Trueの場合限定から全メッセージへ拡張した**: 当初は
    「search_channel_history・search_app_manualは常時提示するツールのため、これらを"required"の
    対象に含めると雑談を含むすべてのメンション応答で毎回1回分余計なツール呼び出しが強制され
    コストが増え続けてしまう」という理由でuse_doc_tools=Falseのときは"required"にしない設計に
    していた。しかしユーザーからの依頼で実施した30問の網羅的な実機検証（既定モデルgpt-5-mini）
    で、「ロールの種類」「ログイン方法」「自分専用DMの作り方」「監査ログの内容」等、search_app_manual
    さえ呼べば正しく答えられる基本的な質問で、ツールを呼ばずに一般的なチャットアプリの知識で
    回答してしまうハルシネーション（実在しない「ゲスト」「モデレーター」ロールの創作、Google認証
    しか無いのに「メールアドレス／パスワードでログイン」という誤案内等）が約10〜20%の頻度で
    再現することが判明し、いずれも「ツールを呼ばなかった場合は必ず不正確、呼んだ場合はほぼ必ず
    正確」という明確な相関を確認した。ユーザーに「コストと引き換えに全メッセージで検索を強制するか」
    を確認し、「全メッセージで検索を強制する（推奨）」を選択されたため、use_doc_tools条件を外し
    全メッセージのラウンド0を強制するよう変更した（雑談等、本来ツール不要な発言でも1回分余計な
    検索が必ず発生するコスト増を受け入れる判断）。
    ...
    for round_num in range(max_rounds + 1):
        ...
        extra = {"tool_choice": "required"} if round_num == 0 else {}
        ...
        if not tool_calls and round_tools is not None and not extra and _looks_like_deferred_search(message.content or ""):
            ...
            # 強制的にやり直す（2026-09-17にラウンド0を全メッセージで常にrequiredにしたため、
            # この分岐は主にラウンド1以降＝"auto"のラウンドで同じ予告だけの失敗が起きた場合の
            # 保険として機能する）
            res = await client.chat.completions.create(
                model=model, messages=messages, max_completion_tokens=MAX_OUTPUT_TOKENS,
