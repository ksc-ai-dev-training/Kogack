...
- Kogack（このチャットアプリ自体）の使い方（メッセージの送り方・書式・メンション・
  ...
  回答を作らず、検索結果に基づいて答えること。search_app_manualは複数件の検索結果を
...
def _build_members_section(members: list[dict], persona_name: str, creator_name: str | None = None) -> str:
    ...
    **バグ修正（ユーザーからの報告「AIに『このチャンネルの名前は？』と聞くと『わかりません』
    ...
    lines.append(
        ...
        f"一切使わず、全員を氏名で呼ぶこと。**参加者について尋ねられたとき限定**の回答例:"
        f"「参加者は{example_names}です」（チャンネル名など参加者以外について尋ねられたときに"
        f"この例文の形をそのまま流用しないこと）"
    )
    return "\n".join(lines)
