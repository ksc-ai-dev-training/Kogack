...
# 2026-09-17続報: 辞書の並び順（＝S-06ドロップダウンの表示順、selectable_models()参照）を、
# 「速度重視（非推論系）→コスト重視（推論系最安）→品質重視（推論系最高性能・既定）」の
# 位置づけに沿ってgpt-4.1-nano/gpt-4.1-mini/gpt-5-nano/gpt-5-miniの順へ並べ替えた
# （単価・特徴のコメント自体は変更していない）。
MODEL_COSTS = {
    ...
    # 選択肢から除外した（実際の応答品質・速度でgpt-4.1-nano系に劣ると判断されたため）。2026-09-17
    # 続報でユーザーからの明示的な要望により既定モデルに変更した（DEFAULT_MODEL参照）。
...
# 2026-09-17続報: 既定をgpt-5-miniへ変更した際、gpt-5-nanoとの実機比較検証（同一のFIXED_RULES・
# 同一の質問セットで34トライアル）を行い、その結果をもとにユーザーと「速度重視ならgpt-4.1-nano/
# gpt-4.1-mini、コスト重視ならgpt-5-nano、それ以外（既定）は品質優先でgpt-5-mini」という
# 位置づけを整理した。gpt-5-nanoはgpt-4.1-nanoより誤情報は少ないが、内部プロンプト構造の
# 漏れ出し（チャンネル名の質問で6回中5回再現）や不要なヘッジ表現が残ることを実機確認済み。
MODEL_DESCRIPTIONS = {
    "gpt-4.1-nano": "非推論系で高速。速度を最優先したいときに（応答品質は他の選択肢より劣る）",
    "gpt-4.1-mini": "非推論系で高速。gpt-4.1-nanoより高性能で、速度を保ちつつ質も上げたいときに",
    "gpt-5-nano": "推論系で最安。コストを最優先したいときに（応答はやや遅く、品質もgpt-5-miniに劣る）",
    "gpt-5-mini": "現在の既定。推論系で最も高性能。速度・コストより回答の正確さを優先したいときに",
}
