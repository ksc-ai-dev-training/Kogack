...
async def update_general(
    ...
    """A-24: AI有効/無効の切り替え・反応モード（F-15）・使用モデル（2026-09-17）の変更。基本設定
    タブ（GeneralTab、AI有効/無効・モデル選択の両方）と反応モードタブ（ReactionTab）はいずれも
    このAPIを呼ぶが、A-27参照ドキュメント範囲と同じく自分が変更しない側のフィールドも現在値の
    まま一緒に送る（差分計算はしない）"""
