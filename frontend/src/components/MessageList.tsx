  ...
  splitCodeBlocks(body).forEach((seg, segIdx) => {
    if (seg.type === 'code') {
      nodes.push(
        <pre
          key={`code-${segIdx}`}
          className="my-1 min-h-[38px] overflow-x-auto whitespace-pre rounded-md border border-line bg-surface-muted px-2.5 py-2 font-mono text-[12.5px] leading-[1.6] text-code-text empty:before:content-['']"
        >
          {seg.content}
        </pre>,
      )
        ...
        nodes.push(
          <div key={`list-${segIdx}-${lsIdx}`} className="my-1 space-y-0.5">
            {ls.items.map((item, ii) => (
              ...
              //
              // バグ修正（ユーザーからの報告「Ctrl+Cだといけるが、右クリックからのコピーだと
              // -になる」）: 最初の実装はdisplay:flexで黒丸とテキストを別spanに分けていたが、
              // 選択範囲をコピーするとブラウザがflexの子要素ごとに改行を挟んでしまっていたため、
              // text-indent（負の値）+padding-leftの「ぶら下げインデント」というCSSテクニックに
              // 変更していた。しかしこの負のtext-indentは、いわゆる「HTMLの見た目をMarkdown風の
              // プレーンテキストへ変換する」処理（多くのブラウザ・OSが右クリックメニューの
              // 「コピー」やスクリーンリーダー向けの読み上げ・抽出機能で使う、Ctrl+Cの単純な
              // 選択範囲テキスト抽出とは別の変換パス）が「ぶら下げインデント＝リストの一種」と
              // 認識し、実際のDOMテキストである「•」を無視して独自に「-」マーカーを生成して
              // しまうことが分かった（Ctrl+Cでは起きずコンテキストメニューのコピーでだけ起きる
              // という報告から、単純な選択範囲コピーとは異なる変換パスが介在していると判断）。
              // text-indentを一切使わない、より素朴な構造（「• 」を含む1本のインラインテキスト
              // として描画するだけ）に変更し、この手のヒューリスティックに一切引っかからない
              // ようにする（本文が折り返した2行目以降がぶら下げインデントにならず左端に揃う
              // 見た目の変化は許容する——コピー結果の正しさを優先する）。
              <div key={ii}>
