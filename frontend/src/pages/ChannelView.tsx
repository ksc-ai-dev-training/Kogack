...
export default function ChannelView() {
  ...
  if (channelError instanceof ApiError && (channelError.status === 404 || channelError.status === 403)) {
    ...
    // 404で返しており区別する情報を持たない）。文言は「削除されている」と断定せず「削除された
    // か、参加権限がありません」の両論併記にした（ユーザーの指摘どおり、存在する非公開
    // チャンネルに対して「削除されています」と言い切るのは不正確なため）。従来は無言でこの
    // 画面が空白のままだったが、削除済みチャンネルのURLへうっかりアクセスした利用者に何の
    // フィードバックも無く分かりづらいとの指摘を受け、明示的なメッセージに差し替えた
    // （無言リダイレクトにはしない）
    return (
      <div className="flex h-full flex-col items-center justify-center gap-2.5 px-6 text-center">
        <div className="text-[15px] font-bold text-ink">このチャンネルは削除されたか、参加権限がありません</div>
        <p className="max-w-[420px] text-[12.5px] leading-relaxed text-ink-subtle">
          このチャンネルはすでに削除されているか、参加する権限がありません。心当たりがある場合はチャンネル管理者にご確認ください。
        </p>
        <Link
          to="/"
