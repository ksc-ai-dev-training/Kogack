...
export default function ChannelView() {
  ...
  useEffect(() => {
    ...
    // 自動スクロールは変わらず機能する
    //
    // バグ修正（2026-09-25）: 依存配列からthreadIdを外した。少し前までさかのぼって読んでいる
    // 状態でスレッドを開閉すると（新着・本文更新は起きていないのに）threadId自体の変化で
    // この効果が再発火し、上のearly returnはhighlightId未設定時は素通りするため無条件に
    // 末尾へ強制スクロールされてしまっていた（ユーザーからの報告「スレッドを表示したときに
    // チャンネル会話が自動的に最新のところまで移動する」）。early return内の判定で現在の
    // threadIdは引き続き参照するが、それ自体の変化では再実行させない。effectが実際に
    // 実行されるのは最後のメッセージのupdated_atが変わったときだけになり、その時点の
    // クロージャは常にレンダー時点の最新のthreadIdを見るため判定の正しさは変わらない
  }, [messages[messages.length - 1]?.updated_at, highlightId])
